...
function _launch_modal(title_window) {
    ...
    //  se muestra el modal
    jQuery('#modal_conceptos').modal('show', { backdrop: 'static', keyboard: false });
}
...
function _set_close_modal() {
    //  cierra el modal
    jQuery('#modal_conceptos').modal('hide');
}
...
function _consultar_catalogo() {
    ...
    try {
        ...
        if ($.trim($('#cmb_busqueda_nombre :selected').val()) !== '') {
            filtros.Concepto_Id = parseInt($('#cmb_busqueda_nombre :selected').val());
        }
            //  validamos que no este seleccionado, el valor enviado sera cero
        else {
            filtros.Concepto_Id = 0;
        }
        ...
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Conceptos_Controller.asmx/Consultar_Conceptos_Filtros',
            data: $data,
            ...
            success: function (datos) {
                ...
                if (datos !== null) {
                    ...
                    $('#tbl_conceptos').bootstrapTable('load', JSON.parse(datos.d));
                }
            }
        });
...
function _validarDatos_nuevo() {
    ...
    try {
        ...
        if ($('#txt_concepto').val() == '' || $('#txt_concepto').val() == undefined) {
            ...
            _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El nombre del concepto.<br />';
        }
...
function alta() {
    ...
    try {
        ...
        obj.Captura_Manual = $('#chk_manual').prop('checked') ? true : false;
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Conceptos_Controller.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        //  se carga la información 
        obj.Concepto_Id = parseInt($('#txt_concepto_id').val());
        obj.Concepto = $('#txt_concepto').val();
        obj.Estatus = $('#cmb_estatus :selected').val();
        obj.Descripcion = $('#txt_descripcion').val();
        obj.Captura_Manual = $('#chk_manual').prop('checked') ? true : false;
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Conceptos_Controller.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
