...
function _load_vistas() {
    ...
    _launchComponent('vistas/Conceptos/Principal.html', 'Principal');
    _launchComponent('vistas/Conceptos/Modal.html', 'Modal');
}
...
function _inicializar_vista_principal() {
    try {
        ...
        //  se inicializa el control de check
        _inicializar_icheck();
...
function crear_tabla_principal() {
    ...
    try {
        ...
        $('#tbl_conceptos').bootstrapTable('destroy');

        //  se carga la estructura que tendrá la tabla
        $('#tbl_conceptos').bootstrapTable({
            cache: false,
            striped: true,
            pagination: true,
            data: [],
            ...
            columns: [
                ...
                { field: 'Concepto', title: 'Concepto', align: 'left', valign: 'top', width: 250, visible: true, sortable: true },
                { field: 'Estatus', title: 'Estatus', align: 'left', valign: 'top', width: 50, visible: true, sortable: true },
                { field: 'Descripcion', title: 'Descripcion', align: 'left', valign: 'top', width: 50, visible: true, sortable: true },
                { field: 'Captura_Manual', title: 'Captura_Manual', align: 'left', valign: 'top', width: 50, visible: false, sortable: true },
                
                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones += '<div style="display:block"><a class="remove ml10 text-purple" id="' + row.Concepto_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i>&nbsp;<span style="font-size:11px !important;">Editar</span></a></div>';
                        opciones += '</div>';
                ...
                },
                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id="' + row.Concepto_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i>&nbsp;<span style="font-size:11px !important;">Eliminar</span></a></div>';
                        opciones += '</div>';
    ...
    try {
       ...
       =============================================*/
        $('#btn_nuevo').on('click', function (e) {
            ...
            ////  se ejecuta el modal
            _launch_modal('<i class="fa fa-list-alt" style="font-size: 25px; color: #0e62c7;"></i>&nbsp;&nbsp;Alta de concepto');
...
function _load_cmb_filtro_nombre(cmb) {
    try {
        ...
        $('#' + cmb).select2({
            ...
            allowClear: true,
            ajax: {
                url: 'controllers/Conceptos_Controller.asmx/Consultar_Conceptos_Nombre_Combo',
                cache: true,
                dataType: 'json',
                type: "POST",
                delay: 250,
...
function _load_cmb_filtro_estatus(cmb) {
    try {
        ...
        $('#' + cmb).select2({
            ...
            allowClear: true,
            ajax: {
                url: 'controllers/Conceptos_Controller.asmx/Consultar_Conceptos_Estatus_Combo',
                cache: true,
                dataType: 'json',
                type: "POST",
                delay: 250,
...
function btn_editar_click(tab) {
    ...
    $('#txt_concepto_id').val(row.Concepto_Id);
    $('#txt_concepto').val(row.Concepto);
    ...
    if (row.Captura_Manual == true) {
        $('#chk_manual').iCheck('check');
    }
    else {
        $('#chk_manual').iCheck('uncheck');
    }
    ...
    //  se muestra el modal
    _launch_modal('<i class="fa fa-list-alt" style="font-size: 25px; color: #0e62c7;"></i>&nbsp;&nbsp;Actualizar concepto');
}
...
function _clear_all_class_error() {
    ...
    /* =============================================
    --NOMBRE_FUNCIÓN:       modal_conceptos
    ...
    --CAUSA_MODIFICACIÓN:
    =============================================*/
    $('#modal_conceptos input[type=text]').each(function (index, element) {
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                ...
                if (result) {
                    ...
                    obj.Concepto_Id = parseInt(row.Concepto_Id);
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'controllers/Conceptos_Controller.asmx/Eliminar',
                        data: $data,
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
                        async: false,
