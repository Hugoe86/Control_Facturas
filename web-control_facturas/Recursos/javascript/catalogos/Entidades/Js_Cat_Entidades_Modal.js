...
function _launch_modal(title_window) {
    ...
    //  se muestra el modal
    jQuery('#modal_entidades').modal('show', { backdrop: 'static', keyboard: false });
}
...
function _set_close_modal() {
    //  cierra el modal
    jQuery('#modal_entidades').modal('hide');
}
...
function _consultar_catalogo() {
    ...
    try {
        ...
        if ($.trim($('#cmb_busqueda_nombre :selected').val()) !== '') {
            filtros.Entidad_Id = parseInt($('#cmb_busqueda_nombre :selected').val());
        }
        ...
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Entidades_Controller.asmx/Consultar_Entidades_Filtros',
            data: $data,
            ...
            success: function (datos) {
                ...
                if (datos !== null) {
                    ...
                    $('#tbl_entidades').bootstrapTable('load', JSON.parse(datos.d));
                }
            }
        });
...
function _validarDatos_nuevo() {
    ...
    try {
        ...
        if ($('#txt_entidad').val() == '' || $('#txt_entidad').val() == undefined) {

            //  agregamos el estilo al control que no cumple
            _add_class_error("txt_entidad");

            //  se indica el estatus como no cumplido
            _output.Estatus = false;

            //  se asigna el mensaje que se mostrara
            _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El nombre de la entidad.<br />';
        }
...
function alta() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Entidades_Controller.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        //  se carga la información 
        obj.Entidad_Id = parseInt($('#txt_entidad_id').val());
        obj.Entidad = $('#txt_entidad').val();
        obj.Estatus = $('#cmb_estatus :selected').val();
        obj.Nombre = $('#txt_descripcion').val();
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Entidades_Controller.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
