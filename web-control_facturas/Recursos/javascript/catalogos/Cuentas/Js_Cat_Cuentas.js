...
function _load_vistas() {
    ...
    _launchComponent('vistas/Cuentas/Principal.html', 'Principal');
    _launchComponent('vistas/Cuentas/Modal.html', 'Modal');
    _launchComponent('vistas/Cuentas/Relacion.html', 'Relacion');
}
...
function crear_tabla_principal() {
    ...
    try {
        ...
        $('#tbl_cuentas').bootstrapTable('destroy');
        ...
        $('#tbl_cuentas').bootstrapTable({
            ...
            columns: [
                { field: 'Cuenta_Id', title: 'Cuenta_Id', align: 'center', valign: 'top', visible: false },
                { field: 'Cuenta', title: 'Cuenta', align: 'left', valign: 'top', width: 100, visible: true, sortable: true },
              ...
              
                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones += '<div style="display:block"><a class="remove ml10 text-purple" id="' + row.Cuenta_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i>&nbsp;Editar<span style="font-size:11px !important;"></span></a></div>';
                        opciones += '</div>';
...

                {
                    ...
                    formatter: function (value, row) {
                        ...
                        opciones = '<div style=" text-align: center;">';
                        opciones += '<div style="display:block"><a class="remove ml10 text-red" id="' + row.Cuenta_Id + '" href="javascript:void(0)" data-orden=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i>&nbsp;Eliminar<span style="font-size:11px !important;"></span></a></div>';
                        opciones += '</div>';
    ...
    try {
       ...
       =============================================*/
        $('#btn_nuevo').on('click', function (e) {
            ...
            ////  se ejecuta el modal
            _launch_modal('<i class="fa fa-list-alt" style="font-size: 25px; color: #0e62c7;"></i>&nbsp;&nbsp;Alta de cuenta');
...
function _load_cmb_filtro_nombre(cmb) {
    try {
        ...
        $('#' + cmb).select2({
            ...
            allowClear: true,
            ajax: {
                url: 'controllers/Cuentas_Controller.asmx/Consultar_Cuentas_Nombre_Combo',
                cache: true,
                dataType: 'json',
                type: "POST",
                delay: 250,
...
function _load_cmb_filtro_estatus(cmb) {
    try {
        ...
        $('#' + cmb).select2({
            ...
            allowClear: true,
            ajax: {
                url: 'controllers/Cuentas_Controller.asmx/Consultar_Cuentas_Estatus_Combo',
                cache: true,
                dataType: 'json',
                type: "POST",
                delay: 250,
...
function btn_editar_click(tab) {
    ...
    $('#txt_cuenta_id').val(row.Cuenta_Id);
    $('#txt_cuenta').val(row.Cuenta);
    ...
    ////  se carga la entidad
    //$('#cmb_entidad').select2("trigger", "select", {
    //    data: { id: row.Entidad_Id, text: row.Entidad }
    //});
    ...
    //  se muestra el modal
    _launch_modal('<i class="fa fa-list-alt" style="font-size: 25px; color: #0e62c7;"></i>&nbsp;&nbsp;Actualizar cuenta');
}
...
function _clear_all_class_error() {
    ...
    --CAUSA_MODIFICACIÓN:
    =============================================*/
    $('#modal_cuentas input[type=text]').each(function (index, element) {
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                ...
                if (result) {
                    ...
                    obj.Cuenta_Id = parseInt(row.Cuenta_Id);
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'controllers/Cuentas_Controller.asmx/Eliminar',
                        data: $data,
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
                        async: false,
...
function btn_relacionar_click(tab) {
    ...
    //  se carga la información en los controles
    $('#txt_cuenta_relacion_id').val(row.Cuenta_Id);
    ...
    //  se muestra el modal
    _launch_modal_relacion('<i class="fa fa-list-alt" style="font-size: 25px; color: #0e62c7;"></i>&nbsp;&nbsp;Relacionar Cuenta-Entidad [' + row.Cuenta + ']');
}