...
function _launch_modal(title_window) {
    ...
    //  se muestra el modal
    jQuery('#modal_anticipos').modal('show', { backdrop: 'static', keyboard: false });
}
...
function _set_close_modal() {
    //  cierra el modal
    jQuery('#modal_anticipos').modal('hide');
}
...
function _consultar_informacion() {
    ...
    try {
        ...
        if ($.trim($('#cmb_busqueda_nombre :selected').val()) !== '') {
            filtros.Anticipo_Id = parseInt($('#cmb_busqueda_nombre :selected').val());
        }
        ...
        //  filtro para los que ya estan siendo utilizados
        if ($('#chk_utilizados').prop('checked') == true) {
            filtros.Filtro_Usado = 1;
            filtros.Usado = true;
        }
        //  validamos que no se seleciono el filtro, se envio informacion para que en la consulta no se filtre
        else {
            filtros.Filtro_Usado = 0;
            filtros.Usado = false;
        }


        //  filtro para los que tiene saldo
        if ($('#chk_con_saldo').prop('checked') == true) {
            filtros.Filtro_Con_Saldo = 1;
        }
            //  validamos que no se seleciono el filtro, se envio informacion para que en la consulta no se filtre
        else {
            filtros.Filtro_Con_Saldo = 0;
        }
        ...
        jQuery.ajax({
            type: 'POST',
            url: 'controllers/Anticipo_Controller.asmx/Consultar_Anticipos_Filtros',
            data: $data,
            ...
            success: function (datos) {
                ...
                if (datos !== null) {
                    ...
                    $('#tbl_anticipos').bootstrapTable('load', JSON.parse(datos.d));
                }
            }
        });
...
function _validarDatos_nuevo() {
    ...
    try {
        ...
        //  ---------------------------------------------------------------------------------------
        //  ---------------------------------------------------------------------------------------
        //  filtro para el anticipo
        if ($('#txt_anticipo').val() == '' || $('#txt_anticipo').val() == undefined) {

            //  agregamos el estilo al control que no cumple
            _add_class_error("txt_anticipo");

            //  se indica el estatus como no cumplido
            _output.Estatus = false;

            //  se asigna el mensaje que se mostrara
            _output.Mensaje += '&nbsp;<i class="fa fa-angle-double-right"></i>&nbsp;El nombre del anticipo.<br />';
        }


        //  filtro para el monto
        if ($('#txt_monto').val() == '' || $('#txt_monto').val() == undefined) {

            //  agregamos el estilo al control que no cumple
            _add_class_error("txt_monto");
...
function alta() {
    ...
    try {
        ...
        //  se carga la información 
        obj.Anticipo_Id = 0;
        obj.Anticipo = $('#txt_anticipo').val();
        obj.Monto = parseFloat($('#txt_monto').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Anticipo_Controller.asmx/Alta',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
...
function actualizar() {
    ...
    try {
        ...
        //  se carga la información 
        obj.Anticipo_Id = parseInt($('#txt_anticipo_id').val());
        obj.Anticipo = $('#txt_anticipo').val();
        obj.Monto = parseFloat($('#txt_monto').val());
        obj.Estatus = $('#cmb_estatus :selected').val();
        ...
        $.ajax({
            type: 'POST',
            url: 'controllers/Anticipo_Controller.asmx/Actualizar',
            data: $data,
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            async: false,
