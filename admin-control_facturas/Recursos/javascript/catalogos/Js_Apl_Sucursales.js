...
function _estado_inicial() {
    try {
        _habilitar_controles('Inicio');
        _limpiar_controles();
        $('#tbl_sucursales').bootstrapTable('refresh', 'Frm_Apl_Cat_Empresas_Ctrl.aspx?accion=consultar_unidades');
        _set_location_toolbar();
    } catch (e) {
        _mostrar_mensaje('Informe Técnico', e);
    }
...
function _cargar_tabla() {
    ...
    try {
        $('#tbl_sucursales').bootstrapTable('destroy');
        $('#tbl_sucursales').bootstrapTable({
            cache: false,
            width: 900,
            height: 400,
            striped: true,
            ...
            columns: [
                ...
                { field: 'Direccion', title: 'Direccion', width: 100, align: 'center', valign: 'bottom', sortable: true, clickToSelect: false },
                { field: 'Email', title: 'Email', width: 150, align: 'center', valign: 'bottom', sortable: true, clickToSelect: false },
                { field: 'Descripcion', title: 'Descripcion', width: 250, align: 'center', valign: 'bottom', sortable: true, clickToSelect: false },
                {
                    field: 'Sucursal_ID',
                    title: '',
                    align: 'center',
                    ...
                    formatter: function (value, row) {
                        return '<div> ' +
                            '<a class="remove ml10 edit" id="' + row.Sucursal_ID + '" href="javascript:void(0)" data-sucursal=\'' + JSON.stringify(row) + '\' onclick="btn_editar_click(this);" title="Editar"><i class="glyphicon glyphicon-edit"></i></button>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Sucursal_ID + '" href="javascript:void(0)" data-sucursal=\'' + JSON.stringify(row) + '\' onclick="btn_eliminar_click(this);" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>' +
                            '&nbsp;&nbsp;<a class="remove ml10 delete" id="' + row.Sucursal_ID + '" href="javascript:void(0)" data-sucursal=\'' + JSON.stringify(row) + '\' onclick="btn_agregar_roles_click(this);" title="Agregar roles"><i class="glyphicon glyphicon-plus"></i></a>' +
                            '</div>';
                    }
                }
            ]
...
function _alta_sucursal() {
    ...
    try {
        ...
        $.ajax({
            type: 'POST',
            url: 'controller/Sucursales_Controllers.asmx/Alta',
            data: $data,
...
function _actualizar_sucursal() {
    ...
    try {
        Sucursal = new Object();
        Sucursal.Sucursal_ID = parseInt($('#txt_sucursal_id').val());
        Sucursal.Clave = $('#txt_clave').val();
        Sucursal.Nombre = $('#txt_nombre').val();
        Sucursal.Direccion = $('#txt_direccion').val();
        Sucursal.Estatus_ID = parseInt($('#cmb_estatus').val());
        ...
        $.ajax({
            type: 'POST',
            url: 'controller/Sucursales_Controllers.asmx/Actualizar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _eliminar(sucursal_id) {
    ...
    try {
        Sucursal = new Object();
        Sucursal.Sucursal_ID = parseInt(sucursal_id);
        var $data = JSON.stringify({ 'jsonObject': JSON.stringify(Sucursal) });

        $.ajax({
            type: 'POST',
            url: 'controller/Sucursales_Controllers.asmx/Eliminar',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _modal() {
    ...
    try {
        ...
        tags +=
            ...
            '        <input type="text" id="txt_nombre" name="txt_nombre" class="form-control input-sm" disabled="disabled" placeholder="(*) Nombre de sucursal" data-parsley-required="true" maxlength="100" required />' +
            '    </div>' +
            '    <div class="col-sm-4">' +
            '       <label class="fuente_lbl_controles">(*) Clave</label>' +
            '        <input type="text" id="txt_clave" name="txt_clave" class="form-control input-sm" disabled="disabled" placeholder="(*) Clave" data-parsley-required="true" maxlength="9" required />' +
            ...
            '        <input type="text" id="txt_cp" name="txt_cp" class="form-control input-sm" disabled="disabled" placeholder="Código postal" data-parsley-required="false" maxlength="20" pattern="[\d]{5}"/>' +
            '    </div>' +
            '    <div class="col-sm-4">' +
            ...
            '        <input type="text" id="txt_ciudad" name="txt_ciudad" class="form-control input-sm" disabled="disabled" placeholder="Ciudad" data-parsley-required="false" maxlength="50" />' +
            '    </div>' +
            '    <div class="col-sm-4">' +
            '       <label class="fuente_lbl_controles">Estado</label>' +
            '        <input type="text" id="txt_estado" name="txt_estado" class="form-control input-sm" disabled="disabled" placeholder="Estado" data-parsley-required="false" maxlength="50" />' +
            '    </div>' +
            '    <div class="col-sm-12">' +
...
function btn_editar_click(empresa) {
    ...
    $('#txt_sucursal_id').val(row.Sucursal_ID);
...
function btn_agregar_roles_click(sucursal) {
    var row = $(sucursal).data('sucursal');
    $('#txt_sucursal').val(row.Sucursal_ID);
    ...
    try {
        show_loading_bar({
            ...
            finish: function (pct) {
                ...
                $.ajax({
                    url: 'controller/Sucursales_Controllers.asmx/Consultar_Sucursales_Por_Filtros',
                    data: $data,
                    method: 'POST',
                    cache: false,
                    async: true,
                    ...
                    success: function (datos) {
                        if (datos !== null) {
                            $('#tbl_sucursales').bootstrapTable('load', JSON.parse(datos.d));
                            hide_loading_bar();
                        }
                    }
                });
...
function _validate_fields(value, id, field) {
    ...
    try {
        Sucursal = new Object();
        if (id !== null)
            Sucursal.Sucursal_ID = parseInt(id);
        ...
        $.ajax({
            type: 'POST',
            url: 'controller/Sucursales_Controllers.asmx/Consultar_Sucursales_Por_Clave',
            data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
...
function _load_estatus() {
    ...
    try {
        jQuery.ajax({
            type: 'POST',
            url: 'controller/Sucursales_Controllers.asmx/ConsultarEstatus',
            //data: $data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
